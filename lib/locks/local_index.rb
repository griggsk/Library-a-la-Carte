...
module ActsAsFerret
  class LocalIndex < AbstractIndex
    ...
    def rebuild_index
       File.open("#{RAILS_ROOT}/index/#{RAILS_ENV}/lockfile","a") do |file|
         ...
         if has_lock  
            ...
            reopen!
            file.flock(File::LOCK_UN) #release the lock on the lockfile
         else
            logger.debug "Unable to gain access to the lockfile"
         end   
      end      
    ...
    #attempts to gain access to the lock
    def get_lock(file)
      attempts = 0 #count of number of tries to gain lock
      while(!(has_lock = file.flock(File::LOCK_EX | File::LOCK_NB)) and attempts < 30)
        sleep((1 + rand(5).to_f)/4)#randomize the sleep number so that processes don't block each other
        attempts += 1
      end  
      return has_lock,file
    end
