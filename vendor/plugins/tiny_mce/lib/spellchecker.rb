...
module TinyMCE
  ...
  module SpellChecker
    ...
    #added this regular expression to handle cases that didn't have a word list included with it
    ASPELL_OTHER = Regexp.new(/\#\s\w+\s\d+/)
    ...
    # added a path for windows users to use
    aspell_path = nil
    ['/usr/bin/aspell', '/usr/local/bin/aspell','c:\\Aspell\\bin\\aspell'].each do |path|
      if File.exists?(path)
        aspell_path = path
        break
      end
    ...
    def check_spelling(spell_check_text, command, lang)
      ...
      for error in spelling_errors
        error.strip!
        if ((match_data = error.match(ASPELL_WORD_DATA_REGEX))||(match_data = error.match(ASPELL_OTHER)))
